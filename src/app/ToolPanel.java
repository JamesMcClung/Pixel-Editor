...
public class ToolPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	public ToolPanel(App app) {
		...
		var bucketButton = new ToolButton("Fill", new Fill(), "F");
		...
		var lightenButton = new ToolButton("Lighten", new Shader(255), "L");
		var darkenButton = new ToolButton("Darken", new Shader(0), "K");
		...
		GBC.addComp(toolButtonPanel::add, i = 0, ++j, lightenButton, toolButtonGBC);
		GBC.addComp(toolButtonPanel::add, ++i, j, darkenButton, toolButtonGBC);
		GBC.addComp(toolButtonPanel::add, ++i, j, smudgeButton, toolButtonGBC);
		...
		// add panels
		GBC.addComp(this::add, 0, 0, sliderPanel,
				new GBC().insets(pad, pad, hpad, pad).anchor(GBC.SOUTH).fill(GBC.HORIZONTAL).weight(0, 1));
		GBC.addComp(this::add, 0, 1, new JSeparator(SwingConstants.HORIZONTAL),
				new GBC().fill(GBC.HORIZONTAL).weight(1, 0).insets(0, hpad, 0, hpad));
		GBC.addComp(this::add, 0, 2, toolButtonPanel,
				new GBC().insets(hpad, pad, pad, pad).fill(GBC.HORIZONTAL).anchor(GBC.NORTH).weight(0, 1));
		GBC.addComp(this::add, 0, 3, pixelCoordDisplay,
				new GBC().insets(hpad, pad, pad, pad).fill(GBC.HORIZONTAL).anchor(GBC.SOUTH).weight(0, 1));
	...
	private Tool.ToolParams getToolParams() {
		...
		if (mousePos == null)
			return null;
		MouseEvent e = new MouseEvent(app.canvasPanel, MouseEvent.MOUSE_MOVED, System.currentTimeMillis(), 0,
	...
	public void mouseClicked(MouseEvent e) {
		if (canUseTool()
				&& handleToolUse(currentTB.tool.click(getActiveLayer(), getPointOnCanvas(e), getToolParams(e))))
			e.consume();
	}
	...
	public void mousePressed(MouseEvent e) {
		if (canUseTool()
				&& handleToolUse(currentTB.tool.press(getActiveLayer(), getPointOnCanvas(e), getToolParams(e))))
			e.consume();
	}
	...
	public void mouseReleased(MouseEvent e) {
		if (canUseTool()
				&& handleToolUse(currentTB.tool.release(getActiveLayer(), getPointOnCanvas(e), getToolParams(e))))
			e.consume();
	}
	...
	public void mouseEntered(MouseEvent e) {
		if (canUseTool()
				&& handleToolUse(currentTB.tool.enter(getActiveLayer(), getPointOnCanvas(e), getToolParams(e))))
			e.consume();
	}
