...
public class CanvasPanel extends JPanel {
	...
	/**
	 * Cuts out pixels within the given shape from the top layer, putting them in their own special layer.
	 * This layer can be rendered at arbitrary locations; see {@link CanvasPanel#setSelectionLoc(Point)}.
	 * If the given mask is null, the entire top layer is made a selection.
	 * If a selection already exists, the specified region is added to the existing selection.
	...
	public void select(PixelMask mask) {
		...
		Layer newSelection;
		if (hadSelection)
			newSelection = new Layer(Math.max(top.getWidth(), currentSelOffset.x + selection.getWidth()) - newSelOffset.x,
		...
		if (hadSelection) // draw previous selection on top
			newSelection.drawImage(selection.getImage(), Util.difference(getPointOnLayer(selectionLoc, false, true), newSelOffset));
