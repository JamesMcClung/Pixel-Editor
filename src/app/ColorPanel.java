...
public class ColorPanel extends JPanel {
	...
	public ColorPanel() {
		...
		GBC.addComp(this::add, 2, 0, new JSeparator(SwingConstants.VERTICAL),
				new GBC().weight(0, 1).fill(GBC.VERTICAL).insets(hpad, 0, hpad, 0));
	...
	private class ColorPalettePanel extends JPanel {
		...
		private class PaletteEditorPanel extends JPanel {
			...
			public PaletteEditorPanel() {
				...
				renameButton = new Button2Field("Rename",
						(comp) -> GBC.addComp(this::add, 1, 1, comp, new GBC().fill(GBC.HORIZONTAL).weight(0, 1)));
	...
	private class ColorMakerPanel extends JPanel implements ColorListener {
		...
		public ColorMakerPanel() {
			...
			GBC.addComp(this::add, 0, 0, new JLabel("Preview"),
					new GBC().anchor(GBC.SOUTH).weight(0, .5).insets(0, hpad, 0, hpad));
			GBC.addComp(this::add, 0, 1, previewColor,
					new GBC().anchor(GBC.NORTH).weight(0, .5).insets(0, hpad, hpad, hpad));
			GBC.addComp(this::add, 0, 2, variantPanel, new GBC().insets(hpad, hpad, 0, hpad));
			GBC.addComp(this::add, 1, 0, adjusterPanel,
					new GBC().dim(1, 3).fill(GBC.BOTH).weight(1, 1).insets(0, hpad, 0, 0),
	...
	public class ColorSwatch extends JPanel {
		...
		/**
		 * Sets the color. Can be overridden to perform other functionality that would happen during runtime, such as
		 * updating components that display information about this colorswatch.
		 * 
		 * @param c a color
		 */
		public void setColorByUser(Color c) {
