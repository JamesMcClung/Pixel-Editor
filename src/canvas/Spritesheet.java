...
public class Spritesheet extends Layer implements Iterable<Layer> {
	...
	public Spritesheet(Dimension spriteDim, Dimension nSprites) {
		this(null, new BufferedImage(spriteDim.width * nSprites.width, spriteDim.height * nSprites.height,
				BufferedImage.TYPE_INT_ARGB), spriteDim);
	}
	...
	public Spritesheet(File file, BufferedImage image, Dimension spriteDim) {
		super(image);
		this.file = file;
		this.spriteDim = spriteDim == null ? new Dimension(image.getWidth(), image.getHeight())
				: new Dimension(spriteDim);
	}
	...
	/**
	 * Sets the dimension of each sprite to the given dimension. Shunts active sprite position if it would be
	 * invalidated.
	 * 
	...
	public void setSpriteDim(Dimension d) {
		if (d.width > getWidth() || d.height > getHeight())
			throw new RuntimeException("Invalid dimension: %s (spritesheet is dimension %s".formatted(d.toString(),
					getImageDim().toString()));
		spriteDim.setSize(d);

		// move active sprite if necessary
		Dimension newSSDim = getSSDim();
		currentSpriteIndex.move(Math.min(currentSpriteIndex.x, newSSDim.width - 1),
				Math.min(currentSpriteIndex.y, newSSDim.height - 1));
