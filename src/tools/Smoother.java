...
public class Smoother extends Brush {
	...
	private void bringToColor(BufferedImage im, Point p, Color c) {
		double w = (double) currentStrength / maxStrength;
		int rgb = im.getRGB(p.x, p.y);
		int a = (int) ((rgb >>> 24 & 0xff)); // it is annoying when alpha is mixed
		int r = (int) ((rgb >>> 16 & 0xff) * (1-w) + c.getRed() * w);
		int g = (int) ((rgb >>> 8 & 0xff) * (1-w) + c.getGreen() * w);
		int b = (int) ((rgb >>> 0 & 0xff) * (1-w) + c.getBlue() * w);
