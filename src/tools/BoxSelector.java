...
public class BoxSelector extends Tool implements Renderable {
	...
	public void renderAt(Graphics2D g, AffineTransform tf) {
		...
		Point2D p1 = tfInverse.transform(start, null),
				p2 = tfInverse.transform(end, null);
		
		Point topLeft = new Point(Util.floor(Math.min(p1.getX(), p2.getX())), Util.floor(Math.min(p1.getY(), p2.getY())));
		Point bottomRight = new Point(Util.ceil(Math.max(p1.getX(), p2.getX())), Util.ceil(Math.max(p1.getY(), p2.getY())));
